...
Superpowers 使用 `TodoWrite` 追踪任务，但会话结束后这些记录就会消失。[Beads](https://github.com/gastownhall/beads)（Steve Yegge）将其替换为 Dolt 支持的问题追踪器，每个任务都是一个具有基于哈希 ID 的 bead，能够跨越会话边界持久存在。Beads 处理依赖追踪、无冲突多智能体工作的单元级合并、通过 events 表实现的完整审计追踪，以及通过 `bd remember` 持久化学习成果。在每次会话开始时，插件钩子注入一份组合式 beads 上下文——任务状态指引加上精选记忆——使智能体从上次中断的地方继续。
...
| 长期记忆 | `bd remember` + 精选注入 + `bd memories` | 我上周学到了什么？ |
...
**插件接管 beads 钩子。** Beads 的 `bd setup claude` 安装运行 `bd prime` 的钩子。插件也需要注入技能上下文。与其同时触发两者并在冗余上下文上浪费 tokens，插件的钩子承担两项工作——在 8 KB 硬性上限内组合按显著度精选的 beads 上下文，而不是注入完整的 `bd prime` 转储（在 218 条记忆的库上实测减少 91.6%）——并在独立钩子仍然安装时让出自身的 beads 部分并发出警告。
