...
**步骤5——决策捕获。** 当一个选择难以撤销、脱离上下文令人意外且是真实权衡的结果时，智能体会提议将其记录为 `decisions/` 中的架构决策记录（Architecture Decision Record）——一个包含上下文、决策、理由和后果的带时间戳的说明。三个条件必须同时满足，因此常规澄清或范围确认不会变成 ADR。该规则在 `using-superpowers` 中只存在一次，并在实际做出决策的地方被引用：brainstorming、规划、stress-test 以及调试会话中的转折点。
