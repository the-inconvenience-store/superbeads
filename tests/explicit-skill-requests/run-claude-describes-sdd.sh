...
TIMESTAMP=$(date +%s)
OUTPUT_DIR="/tmp/superbeads-tests/${TIMESTAMP}/explicit-skill-requests/claude-describes"
mkdir -p "$OUTPUT_DIR"
...
# Create a plan
cat > "$PROJECT_DIR/.internal/plans/auth-system.md" << 'EOF'
...
claude -p "I have a plan at .internal/plans/auth-system.md. Tell me about my options for executing it, including what subagent-driven-development means and how it works." \
    --model haiku \
