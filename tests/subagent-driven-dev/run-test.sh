...
#   ./run-test.sh svelte-todo --plugin-dir /path/to/superbeads
...
if [[ ! -d "$TEST_DIR" ]]; then
  ...
  find "$SCRIPT_DIR" -maxdepth 1 -mindepth 1 -not -name '*.sh' -not -name '*.md' -printf '%f\n'
  exit 1
fi

# Create timestamped output directory
TIMESTAMP=$(date +%s)
OUTPUT_BASE="/tmp/superbeads-tests/$TIMESTAMP/subagent-driven-development"
...
PROMPT="Execute this plan using superbeads:subagent-driven-development. The plan is at: $PLAN_PATH"
