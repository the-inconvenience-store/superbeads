...
| Check | Harness | How |
|---|---|---|
| Local SessionStart fires after `project-init` | Claude Code | new session in the initialized repo → context shows "Beads Workflow Context"; unrelated repo → none |
| Local SessionStart fires after `project-init` | Codex | trust the project hook, then start a new session → bootstrap injected |
| Project plugin bootstrap + compaction re-injection | OpenCode | run `project-init`, start a new session, then compact → re-injection |
